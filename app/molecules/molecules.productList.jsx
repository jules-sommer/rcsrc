...

    {
        ...
        orderingOptions: {
            ...
            costFunction: (options) => {
                ...
                var qty = options.quantities.selected !== false ? options.quantities.values[options.quantities.selected] : 0;

                if(!qty)
                    return (Math.round(0 * 100) / 100).toFixed(2);;
    
                var cost = qty <= 250 && qty >= 0 ? qty*0.65 : qty*0.50 
    
                if(options.isSterile)
                    cost += 7.50
    
                if(options.addPreservative)
                    cost += 0.50
