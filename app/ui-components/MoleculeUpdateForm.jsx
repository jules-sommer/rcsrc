...
export default function MoleculeUpdateForm(props) {
  const {
    id: idProp,
    molecule: moleculeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = moleculeRecord
      ? { ...initialValues, ...moleculeRecord, checkoutID }
      : initialValues;
    setMolName(cleanValues.molName ?? []);
    setCurrentMolNameValue("");
    setMolCAS(cleanValues.molCAS);
    setMolIUPAC(cleanValues.molIUPAC);
    setMolSMILES(cleanValues.molSMILES);
    setInStock(
      typeof cleanValues.inStock === "string"
        ? cleanValues.inStock
        : JSON.stringify(cleanValues.inStock)
    );
    setCheckoutID(cleanValues.checkoutID);
    setCurrentCheckoutIDValue(undefined);
    setCurrentCheckoutIDDisplayValue("");
    setErrors({});
  };
  const [moleculeRecord, setMoleculeRecord] = React.useState(moleculeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Molecule, idProp)
        : moleculeModelProp;
      setMoleculeRecord(record);
      const checkoutIDRecord = record ? await record.checkoutID : undefined;
      setCheckoutID(checkoutIDRecord);
    };
    queryData();
  }, [idProp, moleculeModelProp]);
  React.useEffect(resetStateValues, [moleculeRecord, checkoutID]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Molecule.copyOf(moleculeRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MoleculeUpdateForm")}
      {...rest}
    >
      <ArrayField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || moleculeModelProp)}
