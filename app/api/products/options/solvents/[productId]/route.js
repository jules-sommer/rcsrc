...
export const GET = async (request, context) => {
    ...
    try {
        ...
        if( res.length === 1 ) {
            ...
            const product = res[0];
            const solvents = product.orderingOptions.solvents;
            ...
            if( solvents ) {
                ...
                try {
                    ...
                    let solventsCollection = client.db('data').collection('solvents');

                    const idArray = solvents.map((id) => new ObjectId(id));

                    const filter = {
                        '_id': { $in: idArray }
                    }

                    const raw = solventsCollection.find(filter).toArray()
