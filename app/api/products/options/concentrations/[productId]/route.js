...
export const GET = async (request, context) => {
    ...
    try {
        ...
        if( res.length === 1 ) {
            ...
            const product = res[0];
            const concentrations = product.orderingOptions.concentrations;
            ...
            if( concentrations ) {
                ...
                try {
                    ...
                    let concentrationsCollection = client.db('data').collection('concentrations');

                    const idArray = concentrations.map((id) => new ObjectId(id));

                    const filter = {
                        '_id': { $in: idArray }
                    }

                    const raw = concentrationsCollection.find(filter).toArray()
